...
| ---                    | ---               | ---           | ---       |
|Please select a variable or text (ex. Hello, {vText})|Select or provide a variable or text value|**Hello** or **{vSomeVariable}**||
|Start from Position (ex, 1, {vPosition})|Indicate the starting position within the string|0 for beginning, 1 for first character, etc.||
|Optional - Length (-1 to keep remainder) (ex. 1, -1, {vLength})|Indicate if only so many characters should be kept|-1 to keep remainder, 1 for 1 position after start index, etc.||
